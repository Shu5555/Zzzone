...
## 概要
Zzzoneは、日々の睡眠を記録・可視化して、さらに全国のユーザーと睡眠時間を競い合うことで、健康的で楽しい睡眠習慣をサポートするモバイルアプリケーションです。新たに、偉人たちの「名言」を集めるガチャ機能や、アプリ内のお知らせ機能が追加され、コレクションとカスタマイズの楽しみが広がりました。
...
- **AIによる睡眠分析:** 5件以上の睡眠記録を元に、AI(Google Gemini 2.5 Pro)があなたの睡眠傾向を分析します。最大30件まで読み込みます。
  - **総合得点:** AIが睡眠の質、量、規則性などを総合的に評価し、100点満点でスコアを算出します。スコアの簡単な理由も表示されます。
  - **分析コメント:** 分析項目に「昼間の眠気」「二度寝」が追加され、よりパーソナルな内容になりました。また、状況に応じて「良い点」「改善点」の項目数が2〜4個に変動します。
...
- **AIアシスタント設定:** 睡眠分析レポートを読み上げるAIアシスタントの個性を設定できます。
  - **口調設定:** AIの口調を、全18種類（デフォルト含む）の中から選択できます。（通常、丁寧、友達風、執事風、ツンデレ、カウンセラー、保育士、研究者、アンドロイド、賢者、おっとり系、クール系、元気いっぱい、お姉さん、天才少女...）
  - **AIに認識される性別:** AIがあなたを認識する際の性別（男性、女性、未記入）を設定できます。ペルソナによっては、この設定に応じて呼びかけなどが変化します。

### 6. ショップ
- **タブ切り替え:** ショップは「背景色」と「ガチャ」、「ポイント」の2つのタブで構成されています。
- **背景色の購入:** スリープコインを使い、ランキング画面で利用できる背景色を購入できます。

### 7. プロフィールとランキング
- **プロフィール画面:** ユーザー名やランキングへの参加設定を管理できます。
- **全国ランキング:**
  - **睡眠時間ランキング:** 全ユーザーのその日の睡眠時間を集計し、ランキング形式で表示します。
  - **AIスコアランキング:** AIが算出した睡眠の「総合得点」を全国のユーザーと競い合うことができます。
...
### 11. クイズ機能
- **日替わりクイズ:** 毎日1問、AI（Google Gemini）が睡眠に関するクイズを出題します。
- **出題フロー:** クイズ画面に移動後、「クイズを出題」ボタンをタップするとクイズが表示されます。
- **回答と解説:** 回答を提出すると、AIが正誤を判定し、詳細な解説とともに結果を表示します。AIの解説はMarkdown形式で整形されます。
- **進捗の保存:** その日のクイズを一度プレイすると、回答、AIによる判定結果、解説がアプリ内に保存され、同日中は何度画面を開き直しても同じ状態が復元されます。
- **報酬:** クイズに正解すると、スリープコインを100枚獲得できます。
...
### 13. バックアップと復元 (Dropbox)
- **Dropbox連携:** ご自身のDropboxアカウントと連携することで、アプリのデータを安全にバックアップできます。
- **バックアップ:** 現在の睡眠記録、名言の取得状況、各種設定を、一つのファイルにまとめてDropboxに保存します。
- **復元:** 機種変更時などに、Dropboxに保存したバックアップファイルからデータを復元し、以前の状態を再現できます。
> **Note:** この機能はモバイル版（Android/iOS）とWeb版の両方で利用できます。ただし、バックアップされるデータの形式が異なるため（モバイル版: ZIP, Web版: JSON）、**モバイル版で作成したバックアップをWeb版で復元したり、その逆を行うことはできません。**同じプラットフォーム間でのみバックアップ・復元が可能です。
...
- **APIクライアント:** `http`
- **環境変数管理:** 
  - **デバッグモード:** `assets/.env` ファイル (`flutter_dotenv`)
  - **リリースモード:** コンパイル時環境変数 (`--dart-define`)

### バックエンド
- **ランキング・通貨・ギフトコード管理:** Supabase (PostgreSQL)
- **AI分析機能:** Google Gemini 2.5 Pro API (分析), Gemini 2.5 Flash (クイズ)
